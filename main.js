...
function createEvent(e, i) {
  ...
  return `<article class="bg-white dark:bg-slate-800 shadow-xl shadow-slate-200 dark:shadow-slate-800 rounded-lg">
          ...
          <div class="p-4 md:p-6 lg:p-8 grid gap-4 md:gap-6">
            ...
            <a href="${
              e.link
            }" class="bg-${colorScheme}-500 rounded-md px-4 py-2 text-${colorScheme}-50 shadow-2xl shadow-${colorScheme}-200 dark:shadow-none text-center font-bold hover:shadow-none ring ring-offset-0 ring-${colorScheme}-500 focus:outline-none focus:ring-offset-2">View Event</a>
          </div>
        </article>`
}
...
async function loadEvents(max = 8) {
  try {
    ...
    eventsContainer.innerHTML = processedEvents
